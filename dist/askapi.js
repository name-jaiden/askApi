...
(function (global, factory) {
    ...
    * */
    function searchData(targetPath, data) {
        try {
            return targetPath.split('.').reduce(function (total, key) {
                return total[key];
            }, data)
        } catch (e) {
    ...
    function constructRequestParameters(ref) {
        ...
        if (url.includes(':')) {
            Object.entries(data).map(function (ref) {
                ...
                url = url.replace(new RegExp(("/:(" + key + ")"), 'g'), function (m, k) {
                    return delete data[k] && ("/" + value);
                });
            });
        }
    ...
    * */
    function createAxios(ref) {
        var createDefaultConfig = ref.createDefaultConfig;
        if (createDefaultConfig === void 0) createDefaultConfig = {};
        var interceptors = ref.interceptors;
        if (interceptors === void 0) interceptors = {};

        var _axios = axios.create(createDefaultConfig);
        if (Object.keys(interceptors).length !== 0) {
            var defFn = function (f) {
                return f;
            };
            var defErr = function (e) {
                return Promise.reject(e);
            };
            var request = interceptors.request;
            if (request === void 0) request = defFn;
            var response = interceptors.response;
            if (response === void 0) response = defFn;
            var errorRequest = interceptors.errorRequest;
            if (errorRequest === void 0) errorRequest = defErr;
            var errorResponse = interceptors.errorResponse;
            if (errorResponse === void 0) errorResponse = defErr;
    ...
    * */
    ...
    var AskApi = function AskApi(ref) {
        var axiosConfig = ref.axiosConfig;
        if (axiosConfig === void 0) axiosConfig = {};
        var askConfig = ref.askConfig;
        if (askConfig === void 0) askConfig = {};
